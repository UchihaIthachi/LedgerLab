...
export interface BlockDetailModalProps {
  ...
  editingCoinbaseState?: {
    [blockId: string]: Partial<CoinbaseTransactionType>;
  }; // blockId here is the actual block.id
  ...
  return (
    ...
    >
      <BlockCard
        ...
        onDataChange={
          blockDataType === "simple_text" && onSimpleDataChange
            ? (e) => onSimpleDataChange(e.target.value)
            : undefined
        }
        onNonceChange={onNonceChange}
        onMine={onMine ?? (() => {})}
        isMining={miningState ?? false}
          ...
          >
            ...
            <Space direction="vertical" style={{ width: "100%" }}>
              <Typography.Text strong>
                {t("EditCoinbaseTx", "Edit Coinbase Tx")}
              </Typography.Text>
              <Input
                addonBefore={t("To")}
              ...
              <InputNumber
                ...
                value={Number(
                  editingCoinbaseState[block.id]?.value ?? block.coinbase.value
                )}
              ...
              {onApplyCoinbaseChanges && (
                <AntButton
                  onClick={onApplyCoinbaseChanges}
                  type="dashed"
                  size="small"
                >
      ...
      {(blockDataType === "transactions_with_coinbase" ||
        ...
        onP2PTransactionChange && (
          <Card
            size="small"
            key={`${block.id}-p2p-edit-modal`}
            style={{ marginTop: "10px" }}
          >
            {" "}
            {/* Removed backgroundColor */}
            <Typography.Text strong>
              {t("EditP2PTxs", "Edit P2P Txs")}
            </Typography.Text>
            {(block.data as TransactionType[]).map((tx, txIndex) => (
              <Space
                ...
                style={{
                  ...
                  borderTop:
                    txIndex > 0
                      ? "1px dashed var(--border-color-standard)"
                      : "none",
                }}
              ...
              >
                <Input
                  ...
                  onChange={(e) =>
                    onP2PTransactionChange(tx.id, "from", e.target.value)
                  }
                ...
                <Input
                  ...
                  onChange={(e) =>
                    onP2PTransactionChange(tx.id, "to", e.target.value)
                  }
                ...
                <InputNumber
                  ...
                  onChange={(value) =>
                    onP2PTransactionChange(tx.id, "value", value ?? 0)
                  }
                  style={{ width: "100%" }}
                  min={0}
                ...
                {onApplyP2PTxChanges && (
                  <AntButton
                    onClick={() => onApplyP2PTxChanges(tx.id)}
                    type="dashed"
                    size="small"
                  >
