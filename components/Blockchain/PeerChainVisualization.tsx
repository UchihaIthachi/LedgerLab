...
import { PlusOutlined, ReloadOutlined, ExpandAltOutlined } from '@ant-design/icons';
...
const InnerCanvasAndControls: React.FC<PeerChainVisualizationProps> = (props) => {
  const { peerId, chain, nodeType, miningStates, onShowBlockModal, onAddBlock, onResetChain } = props;
  const { t } = useTranslation('common');
  const { token } = theme.useToken();
  const { fitView } = useReactFlow();

  const [nodes, setNodes, onNodesChange] = useNodesState<CoinbaseFlowNodeData | FlowNodeBlockData>([]);
  ...
  useEffect(() => {
    const newNodes: Node<CoinbaseFlowNodeData | FlowNodeBlockData>[] = chain.map((block, index) => {
      ...
      if (nodeType === 'coinbaseBlock' || nodeType === 'tokenBlock') {
        nodeData = {
          ...
          coinbaseTx: block.coinbase, // May be undefined for tokenBlock if not applicable
        ...
        nodeData = {
          ...
          data: typeof block.data === 'string' ? block.data : JSON.stringify(block.data), // genericBlock expects data as string
          currentHash: block.currentHash,
          previousHash: block.previousHash,
          isValid: block.isValid,
          onClick: () => onShowBlockModal(block),
    ...
    for (let i = 0; i < chain.length - 1; i++) {
      ...
      const isEdgeRelatedToMining = miningStates[sourceBlock.id] || miningStates[targetBlock.id];
    ...
    setNodes(newNodes);
    setEdges(newEdges);
  }, [chain, peerId, nodeType, onShowBlockModal, miningStates, setNodes, setEdges]);
  ...
  return (
    <>
      <div style={{ position: 'absolute', top: '8px', right: '8px', zIndex: 10, display: 'flex', gap: '8px' }}>
        <Button size="small" icon={<PlusOutlined />} onClick={onAddBlock} title={t('AddBlock', 'Add Block')} />
        <Button size="small" icon={<ReloadOutlined />} onClick={onResetChain} title={t('ResetChain', 'Reset Chain')} />
      ...
      >
        <Controls showInteractive={false} /> {/* Basic controls, non-interactive for cleaner look */}
...
const PeerChainVisualization: React.FC<PeerChainVisualizationProps> = React.memo((props) => {
