...
const CoinbasePage: NextPage = () => {
  ...
  return (
    <>
      <Head>
        <title>
          {t("Coinbase", "Coinbase")} - {t("Blockchain Demo")}
        </title>
      </Head>
      <div>
                ...
                {peer.chain.map((block, index) => (
                  <div key={`${peer.peerId}-${block.id}`}>
                    ...
                    {block.coinbase && (
                      <Card
                        size="small"
                        key={`${block.id}-cb-edit`}
                        style={{ marginTop: "5px", backgroundColor: "#fafafa" }}
                      >
                        <Space direction="vertical" style={{ width: "100%" }}>
                          <Text strong>
                          ...
                          <Input
                            addonBefore={t("To")}
                            value={
                              editingCoinbaseState[block.id]?.to ??
                              block.coinbase.to
                            }
                            onChange={(e) =>
                              handleCoinbaseInputChange(
                                block.id,
                                "to",
                                e.target.value
                              )
                          ...
                          <InputNumber
                            ...
                            value={Number(
                              editingCoinbaseState[block.id]?.value ??
                                block.coinbase.value
                            )}
                            onChange={(value) =>
                              handleCoinbaseInputChange(
                                block.id,
                          ...
                          <AntButton
                            onClick={() =>
                              applyCoinbaseChanges(peer.peerId, block.id)
                            }
                            type="dashed"
                            size="small"
                          >
                            {t(
                              "ApplyCoinbaseChanges",
                              "Apply Coinbase Changes"
                            )}
                          </AntButton>
                        </Space>
                      </Card>
                    ...
                    {Array.isArray(block.data) && block.data.length > 0 && (
                      <Card
                        size="small"
                        key={`${block.id}-p2p-edit`}
                        style={{ marginTop: "5px", backgroundColor: "#f0f0f0" }}
                      >
