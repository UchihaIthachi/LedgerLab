...
const getInitialTransactions = (blockNum: number, peerInitial: string): TransactionType[] => [
  { id: `tx-${peerInitial}-${blockNum}-1`, from: 'Alice', to: 'Bob', value: `${10 * blockNum}${peerInitial}` },
  { id: `tx-${peerInitial}-${blockNum}-2`, from: 'Bob', to: 'Charlie', value: `${5 * blockNum}${peerInitial}` },
];
...
const TokensPage: NextPage = () => {
      ...
      currentPeers.map(peer => {
        if (peer.peerId === peerId) {
          ...
          if (Array.isArray(blockToUpdate.data)) {
            ...
            updatedTransactions[txIndex] = { ...updatedTransactions[txIndex], [field]: newValue };
  ...
  return (
    <>
      <Head>
        <title>{t('Tokens', 'Tokens')} - {t('Blockchain Demo')}</title>
      </Head>
      <div>
              ...
              <Space direction="vertical" style={{ width: '100%' }}>
                {peer.chain.map((block, index) => (
                  <div key={`${peer.peerId}-${block.id}`}>
                    ...
                    {Array.isArray(block.data) && block.data.map(tx => (
                      <Card size="small" key={`${tx.id}-edit`} style={{ marginTop: '5px', backgroundColor: '#fafafa' }}>
                        <Space direction="vertical" style={{width: '100%'}}>
                          <Text strong>{t('EditTransaction', 'Edit Tx')}: {tx.id.substring(0,8)}...</Text>
                          <Input
                            addonBefore={t('From')}
